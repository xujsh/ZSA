...
<div class="container-fluid">
  ...
  <ul class="nav nav-list">
	...
	<li><a href="/Admin/adminlogout/">退出</a></li>
		  ...
		  <div>
			<form action="<?php echo ($appbasepath); ?>Resource/reslist/" method="POST" enctype="multipart/form-data">
				<select name='checkField'>
					<option value='guid' <?php if(($checkField) == "guid"): ?>selected = 'selected'<?php endif; ?>>guid</option>
					<option value='updetail' <?php if(($checkField) == "updetail"): ?>selected = 'selected'<?php endif; ?>>文件描述</option>
				</select>
		...
		<p style="font-size:18px;font-family:'微软雅黑'"><a href="<?php echo ($appbasepath); ?>Resource/addres/">添加资源</a></p>
		  <table class="table">
			<thead>
			<tr>
				<th>描述</th>
			...
			<?php if(is_array($resList)): $i = 0; $__LIST__ = $resList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					<td width="130"><a href="<?php echo ($appbasepath); ?>Resource/view/resourceid/<?php echo ($vo["resourceid"]); ?>"  id="view" title="点击查看详情"><?php echo ($vo["updetail"]); ?></a></td>
					<td width="100"><?php echo ($vo["username"]); ?></td>
					<td width="100"><?php echo ($vo["typename"]); ?></td>
					<td width="140"><?php echo ($vo["createtime"]); ?></td>
					<td width="80">
						<?php if($readonly == 0): ?><a href="<?php echo ($appbasepath); ?>Resource/delres/<?php echo ($vo["resourceid"]); ?>" class="btn small-btn" id="deletebutton">删除</a><?php endif; ?>
					</td>
